...
set output "highres_urms_plot.eps"
...
plot sample\
 [i=1:1] '+' using (invRo_hr[i]):(sqrt(urms_hr[i]**2 + vrms_hr[i]**2)):(sqrt(uerr_hr[i]**2 + verr_hr[i]**2)) w yerrorbars pt 9 ps 2 lw 2 lc rgb "blue" title "HR Fr = 0.18",\
 [i=2:2] '+' using (invRo_hr[i]):(sqrt(urms_hr[i]**2 + vrms_hr[i]**2)):(sqrt(uerr_hr[i]**2 + verr_hr[i]**2)) w yerrorbars pt 9 ps 2 lw 2 lc rgb "red" title "HR Fr = 0.1",\
