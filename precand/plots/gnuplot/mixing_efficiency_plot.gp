...
set output "highres_mixing_plot.eps"
set xlabel "1/Ro"
...
plot sample\
 [i=1:1] '+' using (invRo_hr[i]):(mix_hr[i]):(mixerr_hr[i]) w yerrorbars pt 9 ps 3 lw 2 lc rgb "blue" title "HR Fr = 0.18",\
 [i=2:2] '+' using (invRo_hr[i]):(mix_hr[i]):(mixerr_hr[i]) w yerrorbars pt 9 ps 3 lw 2 lc rgb "red" title "HR Fr = 0.1",\
