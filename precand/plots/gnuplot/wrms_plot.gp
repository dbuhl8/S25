...
set output "highres_wrms_plot.eps"
set xlabel "1/Ro"
...
plot sample \
 [i=1:1] '+' using (invRo_hr[i]):(wrms_hr[i]):(werr_hr[i]) w yerrorbars pt 9 ps 2 lw 2 lc rgb "blue" title "HR Fr = 0.18",\
 [i=2:2] '+' using (invRo_hr[i]):(wrms_hr[i]):(werr_hr[i]) w yerrorbars pt 9 ps 2 lw 2 lc rgb "red" title "HR Fr = 0.1",\
