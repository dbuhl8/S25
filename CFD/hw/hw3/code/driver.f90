...
program driver
  ...
  !real(kind=kr), parameter :: fdt = 0.00625
  ...
  print *, " "
    ...
    do j = 1, 6
      ...
      ! running timestepper
      do i = 1, Nt-1
        u(:,i+1) = LF_update_1D(u(:,i),a,dt,dx,Nx)
      end do 
      write(fn,*) Nx, Nt
  ...
  print *, " ----------------------------------------------------------- "
  print *, " "
  print *, " Problem 6-7 Reference:"
  print *, " "

    open(fn, file='prob67check.dat')
    cfl_array = (/0.8, 0.8, 1.0, 1.0, 1.2, 1.2/)
    nx_array = (/32, 128, 32, 128, 32, 128/)
    xstart = 0
    xstop = 1 
    ...
    do j = 1, 6
      ...
      ! running timestepper
      do i = 1, Nt-1
        u(:,i+1) = Upwind_update_1D(u(:,i),a,dt,dx,Nx)
      end do 
      write(fn,*) Nx, Nt
      call writemat(u, Nx, Nt, fn)
      write(fn,*) ""
  ...
  print *, " ----------------------------------------------------------- "
  print *, " "
  print *, " Problem 8-9 Reference:"
  print *, " "

    open(fn,file='prob89check.dat')
    cfl_array = (/0.8, 0.8, 1.0, 1.0, 1.2, 1.2/)
    nx_array = (/32, 128, 32, 128, 32, 128/)
    xstart = -1
    xstop = 1 
